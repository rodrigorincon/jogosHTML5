...
function desenhaQuadro(){
	...
	context.fillRect(canvas.width/4, canvas.height/4, canvas.width/2, canvas.width/2+30);
...
function ranking(cv, ctx, nomeJogo, pontuacao){
	canvas = cv, context = ctx;
	txtQuadroX = canvas.width/4 + 30;
	txtQuadroY = canvas.height/4 + 30;
	desenhaQuadro();
		
	//caso já exista uma lista (não seja a 1ª vez a ser executada)
	if( JSON.parse(localStorage.getItem(nomeJogo+"rank0")) ){
		var i=0, rank = new Array(), posicaoAtual, numNovoValor=-1, maior=false;
		...
		while(posicaoAtual = JSON.parse(localStorage.getItem(nomeJogo+"rank"+i)) ){
			...
			if(i<10){
				...
				if(!maior && pontuacao > posicaoAtual.pontos){
					numNovoValor = i;
					maior = true;
				}
				i++;
